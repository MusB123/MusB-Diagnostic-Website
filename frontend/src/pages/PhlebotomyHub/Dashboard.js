...
const Dashboard = () => {
  ...
  if (loading) return <div style={{ color: '#818cf8', fontWeight: 800 }}>SYNCING OPS DATA...</div>;
      ...
      <div className="hub-main-grid" style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '2rem' }}>
        <div className="hub-glass-panel" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '24px', padding: '2rem' }}>
          <div className="flex-between" style={{ marginBottom: '2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: '0.8rem' }}>
              <div style={{ cursor: 'pointer', padding: '8px', background: 'rgba(255,255,255,0.05)', borderRadius: '10px' }}><Search size={18} /></div>
              <div style={{ cursor: 'pointer', padding: '8px', background: 'rgba(255,255,255,0.05)', borderRadius: '10px' }}><Filter size={18} /></div>
            </div>
          </div>

          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {stats.pipeline && stats.pipeline.length > 0 ? (
              stats.pipeline.map((order, idx) => {
                const badgeId = (order.id || '').toString().includes('-') 
                  ? order.id.split('-')[1] 
                  : (order.id || '???').toString().slice(-3).toUpperCase();
                
                return (
                  <div key={idx} className="order-item" style={{ padding: '1rem', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      <span style={{ padding: '6px 12px', borderRadius: '100px', fontSize: '0.7rem', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '1px', background: order.status === 'Pending' ? 'rgba(245, 158, 11, 0.15)' : 'rgba(16, 185, 129, 0.15)', color: order.status === 'Pending' ? '#fbbf24' : '#34d399' }}>
                        {order.status || 'Active'}
                      </span>
              ...
              <div style={{ padding: '2rem', textAlign: 'center', color: '#475569', fontWeight: 800, fontSize: '0.9rem', border: '2px dashed rgba(255,255,255,0.05)', borderRadius: '18px' }}>
                NO ACTIVE PIPELINE TASKS FOUND
              </div>
            )}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          ...
          <div className="hub-glass-panel" style={{ background: 'linear-gradient(135deg, rgba(99, 102, 241, 0.15) 0%, rgba(168, 85, 247, 0.15) 100%)', border: '1px solid rgba(168, 85, 247, 0.3)' }}>
            ...
            <button className="hub-btn" style={{ width: '100%', marginTop: '1.5rem', background: '#fff', color: '#0f0f1a', fontWeight: 900, borderRadius: '12px', padding: '0.8rem' }}>
